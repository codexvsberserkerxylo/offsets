// dumped by nick
// date: 2026-07-28 12:21:03
// took 15.4s
// success rate: 97.1%

#include <cstdint>
#include <Windows.h>

static const uintptr_t roblox_base = reinterpret_cast<uintptr_t>(GetModuleHandleA(nullptr));
static const uintptr_t hyperion_base = reinterpret_cast<uintptr_t>(GetModuleHandleA("RobloxPlayerBeta.dll"));

#define REBASE(x) ((x) + roblox_base)
#define HREBASE(x) ((x) + hyperion_base)

enum ReflectionType : uint32_t
{
    Void = 0x0,
    Bool = 0x1,
    Int = 0x2,
    Int64 = 0x3,
    Float = 0x4,
    Double = 0x5,
    String = 0x6,
    ProtectedString = 0x7,
    Instance = 0x8,
    Instances = 0x9,
    Ray = 0xa,
    Vector2 = 0xb,
    Vector3 = 0xc,
    Vector2Int16 = 0xd,
    Vector3Int16 = 0xe,
    Rect2d = 0xf,
    CoordinateFrame = 0x10,
    Color3 = 0x11,
    Color3uint8 = 0x12,
    UDim = 0x13,
    UDim2 = 0x14,
    Faces = 0x15,
    Axes = 0x16,
    Region3 = 0x17,
    Region3Int16 = 0x18,
    CellId = 0x19,
    GuidData = 0x1a,
    PhysicalProperties = 0x1b,
    BrickColor = 0x1c,
    SystemAddress = 0x1d,
    BinaryString = 0x1e,
    Surface = 0x1f,
    CollectionHandle = 0x20,
    Enum = 0x21,
    Property = 0x22,
    Tuple = 0x23,
    ValueArray = 0x24,
    ValueTable = 0x25,
    ValueMap = 0x26,
    Variant = 0x27,
    GenericFunction = 0x28,
    WeakFunctionRef = 0x29,
    ColorSequence = 0x2a,
    ColorSequenceKeypoint = 0x2b,
    NumberRange = 0x2c,
    NumberSequence = 0x2d,
    NumberSequenceKeypoint = 0x2e,
    InputObject = 0x2f,
    Connection = 0x30,
    ContentId = 0x31,
    DescribedBase = 0x32,
    RefType = 0x33,
    QFont = 0x34,
    QDir = 0x35,
    EventInstance = 0x36,
    TweenInfo = 0x37,
    DockWidgetPluginGuiInfo = 0x38,
    PluginDrag = 0x39,
    Random = 0x3a,
    PathWaypoint = 0x3b,
    FloatCurveKey = 0x3c,
    RotationCurveKey = 0x3d,
    ValueCurveKey = 0x3e,
    SharedString = 0x3f,
    DateTime = 0x40,
    RaycastParams = 0x41,
    RaycastResult = 0x42,
    OverlapParams = 0x43,
    LazyTable = 0x44,
    DebugTable = 0x45,
    CatalogSearchParams = 0x46,
    OptionalCoordinateFrame = 0x47,
    CSGPropertyData = 0x48,
    UniqueId = 0x49,
    Font = 0x4a,
    SharedTable = 0x4b,
    SharedTableIterator = 0x4c,
    AnimationMask = 0x4d,
    AnimationPose = 0x4e,
    ClipEvaluator = 0x4f,
    OpenCloudModel = 0x50,
    InstanceRef = 0x51,
    SecurityCapabilities = 0x52,
    ArticulatedJoint = 0x53,
    AnimationContext = 0x54,
    Secret = 0x55,
    Buffer = 0x56,
    Integer = 0x57,
    Path2DControlPoint = 0x58,
    ReplicationPV = 0x59,
    FacsReplicationData = 0x5a,
    AnimationMaskModifier = 0x5b,
    Content = 0x5c,
    NetAssetHandle = 0x5d,
    NetAssetRef = 0x5e,
    Object = 0x5f,
    AdReward = 0x60,
    AssetContentMap = 0x61,
    SlimReplicationData = 0x62,
    User = 0x63,
    WebViewParams = 0x64,
    AnimTrackPlayState = 0x65,
    AnimTrackMetadata = 0x66,
    AnimTrackWeight = 0x67,
}; // enum ReflectionType

namespace Offsets
{
    inline constexpr const char* LiveChannel = "version-145f189a6a974303";

    namespace Hyperion
    {
        const uintptr_t BitMap = HREBASE(0x223bf8);
        const uintptr_t ControlFlowGuard = HREBASE(0x12e9fc0);

        inline constexpr uint8_t ByteShift = 15;
        inline constexpr uint8_t PageShift = 12;
        inline constexpr uint32_t PageSize = 0x1000;
        inline constexpr uint32_t PageMask = 0xfff;
        inline constexpr uint8_t BitMask = 7;

        // other
        static const uint64_t patcheb[] = {
            0x51d77c, 0x847d08, 0x8a9b38, 0x8c4c2c, 0x8e6a60, 
            0x9bc834, 0x9fdf70, 0x146a440
        };
    } // namespace Hyperion

    namespace DataModel
    {
        const uintptr_t DataModelDeleterPointer = REBASE(0x7ead66);
        const uintptr_t FakeDataModelPointer = REBASE(0x7e26978);
    } // namespace DataModel

    namespace ScriptContext
    {
        inline constexpr uintptr_t IsCoreScript = 0x22;
        const uintptr_t Resume = REBASE(0x1e21510);
        inline constexpr uintptr_t ToResume = 0x61b53f6;
    } // namespace ScriptContext

    namespace BasePart
    {
        inline constexpr uintptr_t Overlap = 0x1f0;
        inline constexpr uintptr_t Primitive = 0x128;
    } // namespace BasePart

    namespace Connection
    {
        inline constexpr uintptr_t enabled = 0x20;
        inline constexpr uintptr_t next = 0x10;
        inline constexpr uintptr_t slot_wrapper = 0x30;
        inline constexpr uintptr_t slot_wrapper_self = 0x38;
    } // namespace Connection

    namespace Threads
    {
        inline constexpr uintptr_t weak_thread_live = 0x20;
        inline constexpr uintptr_t weak_thread_live_thread = 0x8;
        inline constexpr uintptr_t weak_thread_node = 0x180;
        inline constexpr uintptr_t weak_thread_ref = 0x8;
    } // namespace Threads

    namespace ByteCode
    {
        inline constexpr uintptr_t ModuleScript = 0x138;
        inline constexpr uintptr_t Script = 0x190;
    } // namespace ByteCode

    namespace RobloxThread
    {
        const uintptr_t GetIdentityStruct = REBASE(0x4c2ce10);
        const uintptr_t GetTlsPointer = REBASE(0xfffffffffc27d8f3);
        const uintptr_t IdentityPtr = REBASE(0x8392b28);
        const uintptr_t Impersonator = REBASE(0x640230);
    } // namespace RobloxThread

    namespace TaskScheduler
    {
        inline constexpr uintptr_t JobName = 0x18;
        inline constexpr uintptr_t JobsEnd = 0xd0;
        inline constexpr uintptr_t JobsStart = 0xc8;
        inline constexpr uintptr_t MaxFps = 0x1d0;
        const uintptr_t Pointer = REBASE(0x84a58e0);
        const uintptr_t TargetFps = REBASE(0x84a5ab0);
    } // namespace TaskScheduler

    namespace Instance
    {
        inline constexpr uintptr_t ClassDescriptor = 0x18;
        const uintptr_t GetProperty = REBASE(0xc8b240);
        inline constexpr uintptr_t getter = 0x18;
        const uintptr_t Push = REBASE(0x1d48ca0);
        inline constexpr uintptr_t scriptable = 0x90;
        inline constexpr uintptr_t ttype = 0x68;
        inline constexpr uintptr_t ttype_number = 0x30;
    } // namespace Instance

    namespace Property
    {
        const uintptr_t HashTableLookup = REBASE(0x5079d10);
        const uintptr_t KTable = REBASE(0x85a7390);
    } // namespace Property

    namespace Raknet
    {
        const uintptr_t HandleConnectionState = REBASE(0xa49490);
        const uintptr_t ProcessNetworkPacket = REBASE(0x324e020);
        const uintptr_t Receive = REBASE(0x32529a0);
        const uintptr_t ReportNetworkError = REBASE(0xa3dea0);
        const uintptr_t Send = REBASE(0x324e650);
    } // namespace Raknet

    namespace Luau
    {
        const uintptr_t currfuncname = REBASE(0x1d22760);
        const uintptr_t f_luaopen = REBASE(0x4b68310);
        const uintptr_t lua_break = REBASE(0x4b683c0);
        const uintptr_t lua_clock = REBASE(0x4b6cd80);
        const uintptr_t lua_exception = REBASE(0x4b674c0);
        const uintptr_t lua_getfield = REBASE(0x4b6a2aa);
        const uintptr_t lua_pushfstringL = REBASE(0x4b631a0);
        const uintptr_t lua_pushstring = REBASE(0x4b63510);
        const uintptr_t lua_pushvfstring = REBASE(0x4b638b0);
        const uintptr_t lua_yield = REBASE(0x4b68620);
        const uintptr_t luaB_assert = REBASE(0x4b7df90);
        const uintptr_t luaB_error = REBASE(0x4b7da50);
        const uintptr_t luaB_gcinfo = REBASE(0x4b7de80);
        const uintptr_t luaB_getfenv = REBASE(0x4b7dbe0);
        const uintptr_t luaB_getmetatable = REBASE(0x3523ea0);
        const uintptr_t luaB_newproxy = REBASE(0x4b7e0b0);
        const uintptr_t luaB_next = REBASE(0x4b7df30);
        const uintptr_t luaB_pcall = REBASE(0x4b622b0);
        const uintptr_t luaB_pcallrun = REBASE(0x4b63030);
        const uintptr_t luaB_print = REBASE(0x4b7d830);
        const uintptr_t luaB_rawequal = REBASE(0x26d0070);
        const uintptr_t luaB_rawget = REBASE(0x4b7dd70);
        const uintptr_t luaB_rawlen = REBASE(0x4b7de20);
        const uintptr_t luaB_rawset = REBASE(0x4b7ddc0);
        const uintptr_t luaB_setfenv = REBASE(0x1e77870);
        const uintptr_t luaB_setmetatable = REBASE(0x4b7db20);
        const uintptr_t luaB_tonumber = REBASE(0x4b7d910);
        const uintptr_t luaB_tostring = REBASE(0x4b7e0b0);
        const uintptr_t luaB_type = REBASE(0x4b7deb0);
        const uintptr_t luaB_typeof = REBASE(0x4b7deb0);
        const uintptr_t luaD_throw = REBASE(0x4b68390);
        const uintptr_t luaF_freeproto = REBASE(0x4b93a80);
        const uintptr_t luaF_newlclosure = REBASE(0x4b93d70);
        const uintptr_t luaF_newproto = REBASE(0x4b93e20);
        const uintptr_t luaG_runerror = REBASE(0x1e63c70);
        const uintptr_t luaG_runerrorl = REBASE(0x4b6b470);
        const uintptr_t luaH_dummynode = REBASE(0x6bc02c8);
        const uintptr_t luaH_new = REBASE(0x4b8c810);
        const uintptr_t luaL_argerrorl = REBASE(0x4b69110);
        const uintptr_t luaL_checkstring = REBASE(0x4b6a2aa);
        const uintptr_t luaL_checktype = REBASE(0x4b69760);
        const uintptr_t luaL_error = REBASE(0x4b69760);
        const uintptr_t luaL_errorl = REBASE(0x4b69760);
        const uintptr_t luaL_findtable = REBASE(0x4b697b0);
        const uintptr_t luaL_getmetafield = REBASE(0x4b61930);
        const uintptr_t luaL_register = REBASE(0x4b69e30);
        const uintptr_t luaL_testudata = REBASE(0x4b956e0);
        const uintptr_t luaL_tostring = REBASE(0x1d22760);
        const uintptr_t luaL_typeerrorL = REBASE(0x4b6a2aa);
        const uintptr_t luaM_free = REBASE(0x4b8a1a0);
        const uintptr_t luaM_freegco = REBASE(0x4b8a940);
        const uintptr_t luaM_toobig = REBASE(0x4b6b410);
        const uintptr_t luaM_visitgco = REBASE(0x4b8a040);
        const uintptr_t luaO_chunkid = REBASE(0x4b99d90);
        const uintptr_t luaO_nilobject = REBASE(0x6bc0438);
        const uintptr_t luaopen_base = REBASE(0x4b7e4b0);
        const uintptr_t luaopen_math = REBASE(0x4af6c90);
        const uintptr_t luaT_eventnames = REBASE(0x6bc0238);
        const uintptr_t luaT_init = REBASE(0x4b8ac90);
        const uintptr_t luaT_typenames = REBASE(0x5f4ede0);
        const uintptr_t luau_execute = REBASE(0x350e5d0);
        const uintptr_t luaV_gettable = REBASE(0x4b95820);
        const uintptr_t luaV_settable = REBASE(0x4b95ee0);
        const uintptr_t luaVM_load = REBASE(0x4b4e4d0);
        const uintptr_t print = REBASE(0x4c27bf0);
        const uintptr_t pseudo2addr = REBASE(0x5154200);
        const uintptr_t pusherror = REBASE(0x4b65ce0);
    } // namespace Luau

    namespace Task
    {
        const uintptr_t cancel = REBASE(0x1e95330);
        const uintptr_t defer = REBASE(0x1e95640);
        const uintptr_t desynchronize = REBASE(0x1e95c80);
        const uintptr_t spawn = REBASE(0x1e96520);
        const uintptr_t synchronize = REBASE(0x1e966a0);
        const uintptr_t wait = REBASE(0x1e968a0);
    } // namespace Task

    namespace Coroutine
    {
        const uintptr_t close = REBASE(0x4b7e830);
        const uintptr_t create = REBASE(0x4b7e710);
        const uintptr_t isyieldable = REBASE(0x4b7e800);
        const uintptr_t resume = REBASE(0x4b7e960);
        const uintptr_t running = REBASE(0x4b7e7d0);
        const uintptr_t status = REBASE(0x4b7e6a0);
        const uintptr_t wrap = REBASE(0x4b7e750);
        const uintptr_t yield = REBASE(0x4b7e7b0);
    } // namespace Coroutine

    namespace Signals
    {
        const uintptr_t FireMouseClick = REBASE(0x26ae550);
        const uintptr_t FireMouseHoverEnter = REBASE(0x26ae0f0);
        const uintptr_t FireMouseHoverLeave = REBASE(0x26ae2e0);
        const uintptr_t FireProximityPrompt = REBASE(0x26e9ec0);
        const uintptr_t FireRightMouseClick = REBASE(0x26adee0);
        const uintptr_t FireTouchInterest = REBASE(0x2b25900);
    } // namespace Signals

    namespace IsParallel
    {
        const uintptr_t ActorRequired_Call = REBASE(0x1d41d10);
        const uintptr_t ActorRequired_Prop = REBASE(0x1d41f56);
        const uintptr_t Connection_Disconnect = REBASE(0x1d5da50);
        const uintptr_t Event_Connect = REBASE(0x1d5c900);
        const uintptr_t Event_ConnectParallel = REBASE(0x1d5cd30);
        const uintptr_t Event_Once = REBASE(0x1d60470);
        const uintptr_t EventAccess = REBASE(0x1d3dc20);
        const uintptr_t FilterDescendants = REBASE(0x1d698e0);
        const uintptr_t FunctionCall = REBASE(0x1d41d10);
        const uintptr_t Instance_fromExisting = REBASE(0x1e013c0);
        const uintptr_t LocalSafe_Call = REBASE(0x1d41d10);
        const uintptr_t LocalSafe_Write = REBASE(0x1d41f56);
        const uintptr_t PropertyRead = REBASE(0x1d3dc20);
        const uintptr_t PropertyWrite = REBASE(0x1d41f56);
        const uintptr_t ScriptConnect = REBASE(0x1d5cd30);
    } // namespace IsParallel

    // other
    const uintptr_t AttachRobloxExtraSpace = REBASE(0x1db73e0);
    const uintptr_t CastArgs = REBASE(0x3ba6b90);
    const uintptr_t closestate = REBASE(0x4b65c20);
    const uintptr_t ConnectionDisconnect = REBASE(0x513fb60);
    const uintptr_t EnableLoadModule = REBASE(0x1e772a0);
    const uintptr_t GetCapabilities = REBASE(0x4c2d260);
    const uintptr_t GetContextObject = REBASE(0x1d66c90);
    const uintptr_t GetCurrentThreadId = REBASE(0x5123f60);
    const uintptr_t GetFFlag = REBASE(0x516c650);
    const uintptr_t GetGlobalState = REBASE(0x5573550);
    const uintptr_t GetLuaStateForInstance = REBASE(0x4b6ee71);
    const uintptr_t loadsafe = REBASE(0x4b6ece0);
    const uintptr_t LockViolationInstanceCrash = REBASE(0x602c688);
    const uintptr_t LockViolationScriptCrash = REBASE(0x6032c78);
    const uintptr_t LuaStepIntervalMsOverrideEnabled = REBASE(0x6042338);
    const uintptr_t PhysicsSenderMaxBandwidthBps = REBASE(0x5ee4e20);
    const uintptr_t Register = REBASE(0x2ca7b50);
    const uintptr_t RobloxLogCrash = REBASE(0x513ab80);
    const uintptr_t SetFFlag = REBASE(0x5172070);
    const uintptr_t WebSocketServiceEnableClientCreation = REBASE(0x5f3a4b0);
    const uintptr_t WndProcessCheck = REBASE(0x5e47170);
} // namespace Offsets

namespace Structs
{
    inline constexpr uintptr_t CommonHeader_memcat = 0x2;
    inline constexpr uintptr_t Proto_code = 0x58;
    inline constexpr uintptr_t Proto_p = 0x68;
    inline constexpr uintptr_t Proto_k = 0x50;
    inline constexpr uintptr_t Proto_lineinfo = 0x30;
    inline constexpr uintptr_t Proto_locvars = 0x48;
    inline constexpr uintptr_t Proto_upvalues = 0x28;
    inline constexpr uintptr_t Proto_debuginsn = 0x28;
} // namespace Structs
